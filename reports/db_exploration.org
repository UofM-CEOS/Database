...
#+TITLE: Exploration of variables 2011
#+OPTIONS: ^:{}
...
SELECT time_study_1min AS time_study,
  ...
  heave_avg as heave
FROM amundsen_flux.navigation_1min_2011
"""
...
FROM amundsen_flux.meteorology_ceos_1min_2011
"""
...
SELECT time_study, "PAR", "K_down", "LW_down",
  "UV_a", "UV_b", "UV_broad"
FROM amundsen_flux.radiation_1min_2011
"""
...
#+BEGIN_SRC python :session :results file :exports results
UVblims = (0, 2)         # UV (W/m2)
UValims = (0, 15)        # UV (W/m2)
UVbrlims = (0, 50)       # UV (W/m2)
fig, axs = plt.subplots(3, 1, sharex=True)
fig.set_size_inches((11, 9))
...
rad[['UV_broad']].plot(ax=axs[2], ylim=UVbrlims, rot=0, legend=False)
axs[2].set_ylabel('UV broad ($W/m^{2}$)'); axs[2].set_xlabel('')
...
FROM amundsen_flux.motion_10hz_2011
...
FROM amundsen_flux.wind3d_analog_1min_2011
"""
...
SELECT time_study_1min as time_study,
  ...
  air_temperature_sonic_avg AS air_temperature_sonic,
  sound_speed_avg AS sound_speed
FROM amundsen_flux.wind3d_serial_1min_2011
"""
wind3d=pd.read_sql(qry, conn, parse_dates=['time_study'],
                   index_col="time_study")
wind3d[['wind_speed_u']].plot(ax=axs[0], legend=False)
...
FROM amundsen_flux.opath_1min_2011
"""
...
qry="""
SELECT time_study_1min as time_study,
  "cp_CO2_fraction_avg" as "cp_CO2_fraction",
  "cp_H2O_fraction_avg" as "cp_H2O_fraction",
...
FROM amundsen_flux.cpath_1min_2011
"""
...
qry="""
SELECT time_study_1min as time_study, equ_temperature,
  "uw_CO2_fraction", "uw_H2O_fraction", uw_temperature_analyzer,
  uw_pressure_analyzer, equ_pressure, "H2O_flow", air_flow_analyzer,
  condensation_equ, ctd_pressure, ctd_temperature, ctd_conductivity,
  "ctd_O2_saturation", "ctd_O2_concentration", temperature_external,
  tsg_temperature, nbad_ctd_flag, "nbad_CO2_flag", "nbad_H2O_flag",
...
FROM amundsen_flux.underway_1min_2011;
"""
