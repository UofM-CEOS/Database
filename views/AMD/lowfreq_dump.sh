...
FROM amundsen_UWpCO2.${LFREQ1}
WHERE uw_record_type != 'EXTERNAL_TEMPERATURE'
ORDER BY time_study;
\cd ${LFREQ1ODIR}
\copy (SELECT * FROM lowfreq_1min) TO 'L1_2018.csv' CSV HEADER DELIMITER E'\t' NULL '-999'
\H
\o colnames.html
...
WHERE pa.attrelid = 'amundsen_flux.lowfreq_1w20min_2018'::regclass AND
